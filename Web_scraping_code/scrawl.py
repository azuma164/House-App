...
for url in urls:
    ...
    for i in range(len(cassetteitems)):
        ...
        # rent取得いらない？
        subrent = cassetteitems[i].find_all("li",{
            'class':'cassetteitem_price cassetteitem_price--rent'})
        subrent = str(subrent)
        subrent_rep = subrent.replace(
            '[<li class="cassetteitem_price cassetteitem_price--rent">', '')
        subrent_rep2 = subrent_rep.replace(
            '</li>]', '')
